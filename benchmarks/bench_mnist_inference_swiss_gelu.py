...
def benchmark_model(model, data_loader, warmup=10, iters=100):
...
def main():
    print("\n" + "="*80)
    print("  MNIST CNN INFERENCE BENCHMARK - FUSION 1: GELU+SWISH")
    print("="*80 + "\n")
    ...
    for batch_size in batch_sizes:
        ...
        # PyTorch unfused
        print("\n1. PyTorch (Unfused)")
        model_unfused = FusedCNN(use_fusion=False).cuda().eval()
        ...
        results.append(("cnn_inference", "pytorch_unfused", batch_size, mean_unfused, std_unfused))
        
        # Triton fused
        print("\n2. Triton (Fused)")
        model_triton = FusedCNN(use_fusion=True, backend='triton').cuda().eval()
        ...
        results.append(("cnn_inference", "triton_fused", batch_size, mean_triton, std_triton))
        
        # CUDA fused
        print("\n3. CUDA (Fused)")
        model_cuda = FusedCNN(use_fusion=True, backend='cuda').cuda().eval()
        ...
        results.append(("cnn_inference", "cuda_fused", batch_size, mean_cuda, std_cuda))
        
        print(f"\nBest speedup: {max(speedup_triton, speedup_cuda):.2f}×\n")
    
    # Save results
    os.makedirs("report", exist_ok=True)
    with open("report/mnist_cnn_fusion1.csv", "w", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["operation", "implementation", "batch_size", "mean_ms", "std_ms"])
        writer.writerows(results)
    
    print("="*80)
    print("✅ Results saved to: report/mnist_cnn_fusion1.csv")
    print("="*80 + "\n")
