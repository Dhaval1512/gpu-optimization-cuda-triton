#!/usr/bin/env python3
"""CNN model using Fusion 2: LayerNorm + Swish + Dropout"""

import torch
import torch.nn as nn
import torch.nn.functional as F

class Fusion2CNN(nn.Module):
    """
    CNN for MNIST demonstrating Fusion 2: LayerNorm + Swish + Dropout
    """
    def __init__(self, use_fusion=False, backend='triton'):
        super(Fusion2CNN, self).__init__()
        ...
        if use_fusion:
            if backend == 'triton':
                ...
                self.fused_ln_swish_dropout = triton_fused_ln_swish_dropout
            elif backend == 'cuda':
                import cuda_ops
                self.fused_ln_swish_dropout = lambda x, g, b, p, t: cuda_ops.cuda_fused_ln_swish_dropout(x, g, b, p, 42, 1e-5)
            else:
    ...
    def forward(self, x):
        ...
        # Reshape for LayerNorm + Swish + Dropout fusion
        B, C, H, W = x.shape
        ...
        if self.use_fusion:
            ...
            if self.backend == 'triton':
                x_flat, _ = self.fused_ln_swish_dropout(
                    x_flat, gamma, beta,
                    dropout_p=0.25,
            ...
            # Unfused: LayerNorm + Swish + Dropout
            x_flat = F.layer_norm(x_flat, (C * H * W,), 
                                 self.ln1.weight.view(-1),
                                 self.ln1.bias.view(-1))
            x_flat = x_flat * torch.sigmoid(x_flat)  # Swish
            x_flat = F.dropout(x_flat, p=0.25, training=self.training)
        ...
        # Apply Fusion 2 to FC layer
        if self.use_fusion:
            gamma = self.ln3.weight
            beta = self.ln3.bias
            
            if self.backend == 'triton':
                x, _ = self.fused_ln_swish_dropout(
                    x, gamma, beta,
                    dropout_p=0.5,
                    training=self.training
                )
...
if __name__ == "__main__":
    # Test model
    print("Testing Fusion2CNN architecture...")
    
    model = Fusion2CNN(use_fusion=False).cuda()
